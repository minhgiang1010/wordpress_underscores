...
function underscores_scripts()
{
	...
	wp_enqueue_script('underscores-nav-menu', get_template_directory_uri() . '/js/nav-menu.js', array(), '20191003', true);

	wp_enqueue_script('underscores-carousel', get_template_directory_uri() . '/js/carousel.js', array(), '20191012', true);
 ...
 */
function underscores_admin_scripts($hook)
{
	if (isset($_GET['post'])) {
		...
		if ($post->post_type == 'cpt-slider' && $hook == 'post.php') {
			wp_register_style('slider_style', get_template_directory_uri() . '/css/slider_admin.css', array(), '20191106', 'all');
			wp_enqueue_style('slider_style');

			wp_enqueue_media();

			wp_register_script('slider-script', get_template_directory_uri() . '/js/slider-admin.js', array('jquery'), '20191107', 'all');
...
add_action('admin_enqueue_scripts', 'underscores_admin_scripts');
...
/**
 * * HuuTien Menu Walker Alecadd Mega 
 */
